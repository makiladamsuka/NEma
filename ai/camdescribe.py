...
ANALYSIS_INTERVAL_SECONDS = 2  # The frequency of the analysis

# 5. Optimization: Frame resolution sent to the LLM (Lower = Faster)
FRAME_WIDTH = 320
FRAME_HEIGHT = 240 
...
if __name__ == "__main__":
    ...
    try:
        print(f"✅ Starting FAST LIVE webcam analysis. Model: {MODEL_NAME}")
        ...
        with open(OUTPUT_LOG_FILE, 'w') as log_file:
            ...
            log_file.write(f"Analysis Resolution: {FRAME_WIDTH}x{FRAME_HEIGHT}\n\n")
            ...
            while True:
                ...
                if not ret:
                    print("\nCamera stream ended or failed.")
                    break
                
                frame_count += 1
                current_time = time.time()
                
                # 3. Time-based check to send a frame to the LLM
                if current_time - last_analysis_time >= ANALYSIS_INTERVAL_SECONDS:
                    last_analysis_time = current_time
                    analyzed_frame_count += 1
                    
                    print(f"⏱️ Capturing frame {frame_count} for analysis...", end='\r')
                    
                    # Resize the frame before encoding to dramatically reduce the payload size
                    analysis_frame = cv2.resize(frame, (FRAME_WIDTH, FRAME_HEIGHT))
                    
                    start_llm_time = time.time()

                    # Encode and send to LLM
                    image_data = encode_frame_to_base64(analysis_frame)
                    ...
                    latency = end_llm_time - start_llm_time
                    
                    log_entry = f"[Frame: {frame_count}, Latency: {latency:.2f}s] -> {llm_answer}\n"
                    log_file.write(log_entry)
                    
                    # Use strip() to clean up the output description
                    print(f"✅ Analysis ({latency:.2f}s): {llm_answer.strip()}")
                
                # We can add a short sleep here to prevent the loop from consuming 100% CPU
                # but we'll rely on the time check to manage the LLM calls.
                time.sleep(0.01) # Sleep for 10ms to yield CPU
        ...
        # cv2.destroyAllWindows() is no longer needed since no windows were created.
