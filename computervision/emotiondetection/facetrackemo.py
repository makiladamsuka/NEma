...
try:
    while True:
        # Read the frame from the camera (BGR format)
        frame = picam2.capture_array()
        
        # Set YuNet input size to the current frame size
        face_detector.setInputSize((FRAME_WIDTH, FRAME_HEIGHT)) 
        
        # --- YuNet Face Detection (Fast) ---
        ...
        if faces is not None:
            ...
            cv2.circle(frame, (face_center_x, face_center_y), 5, (255, 0, 0), -1) # Blue dot at face center
            cv2.putText(frame, emotion_text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, emotion_color, 2)


        # --- Servo Smoothing and Movement ---
        # Apply smoothing filter (linear interpolation)
